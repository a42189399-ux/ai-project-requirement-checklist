...
3.  `examples/ai_project_brief_manifest.json`: Structuring the gathered requirements into a machine-readable JSON schema.
...
1.  **Drafting Clarifying Questions**: Translate the 10 core items in `checklist.md` into plain, user-facing questions. Do not ask all 10 questions at once; ask them in small, logical chunks (e.g., first ask about Inputs/Outputs, then API limits, then Fallback mechanisms).
...
*   **No Pricing Guarantees**: AI agents must **never** estimate or guarantee development costs, developer hourly rates, or project budgets. Suggest they use the Excel model as a rough checklist, but advise them to get multiple quotes.
